...
3. Click **Install BPF Helper** in the app the first time you run it (or re-run the `.pkg`).
...
If you don't need the BPF helper (e.g. you already have Wireshark's, or you're fine running with `sudo`) but you do want `portfinder` on your `PATH`:

```bash
sudo ln -sf /Applications/PortFinder.app/Contents/MacOS/PortFinder /usr/local/bin/portfinder
```

`PortFinder-BPF-<version>.pkg` from the release assets drops the same symlink as part of the install. The standalone `install-cli.sh` / `uninstall-cli.sh` scripts that shipped with 3.x are gone in 4.x — the symlink isn't worth its own bespoke installer.
</details>
