...
Once the BPF helper has run (either via the in-app *Install BPF Helper* button or by installing `PortFinder-BPF-*.pkg`), a symlink at `/usr/local/bin/portfinder` is created automatically — `portfinder` works from any shell.
